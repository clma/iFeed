...
- (void)viewDidLoad
{
    ...
    //监视设备的方向变化
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(phoneSetFrameChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];
    
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;
    
    //创建电话号码输入框
    _textPhone = [[UITextField alloc] initWithFrame:CGRectMake(10, self.navigationController.navigationBar.frame.size.height + 41, mainWidth - 20, 40)];
    ...
    //提示信息
    _labelContent = [[UILabel alloc] initWithFrame:CGRectMake(10, _textPhone.frame.origin.y + _textPhone.frame.size.height + 1, mainHeight - 20, 60)];
...
- (void)phoneSetFrameChanged:(NSNotification*)notification
{
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;
    
    _textPhone.frame = CGRectMake(10, self.navigationController.navigationBar.frame.size.height + 41, mainWidth - 20, 40);
    _labelContent.frame = CGRectMake(10, _textPhone.frame.origin.y + _textPhone.frame.size.height + 1, mainHeight - 20, 60);
