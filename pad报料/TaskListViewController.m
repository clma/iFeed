...
//    [btnEdit release];
    ...
    //监视设备的方向变化
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ListFrameChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];
    ...
    [_segmentedController setWidth:120.0f forSegmentAtIndex:0];
    [_segmentedController setWidth:120.0f forSegmentAtIndex:1];
    [_segmentedController setWidth:120.0f forSegmentAtIndex:2];
    ...
    NSLog(@"mainWidth:%f,navBar:%f,tabbar:%f", mainWidth, self.navigationController.navigationBar.frame.size.height, self.tabBarController.tabBar.frame.size.height);
    
    //创建scroll页面
    CGFloat scrollHeight = mainHeight - self.navigationController.navigationBar.frame.size.height - 20 - self.tabBarController.tabBar.frame.size.height;
    _taskScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.navigationController.navigationBar.frame.size.height + 20, mainWidth, scrollHeight)];
    ...
    //_taskScrollView.backgroundColor = [UIColor whiteColor];
    _taskScrollView.delegate = self;
    [self.view addSubview:_taskScrollView];
    
    //NSLog(@"_taskScrollView:%@", _taskScrollView);
    ...
    _onTaskTableView.rowHeight = 80.0f;
    //_onTaskTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    ...
    //_saveTaskTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    _saveTaskTableView.rowHeight = 80.0f;
    ...
    //_doneTaskTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    _doneTaskTableView.rowHeight = 80.0f;
...
{    
    ...
    CGFloat scrollHeight = mainHeight - self.navigationController.navigationBar.frame.size.height - 20 - self.tabBarController.tabBar.frame.size.height;
    _taskScrollView.frame = CGRectMake(0, self.navigationController.navigationBar.frame.size.height + 20, mainWidth, scrollHeight);
    _taskScrollView.contentSize = CGSizeMake((_taskScrollView.frame.size.width)*3, _taskScrollView.frame.size.height);
    //这个设置非常重要，不能少
    if (_segmentedController.selectedSegmentIndex == 0) {
        _taskScrollView.contentOffset = CGPointMake(0, -self.navigationController.navigationBar.frame.size.height - 20);
    }
    else if (_segmentedController.selectedSegmentIndex == 1){
        _taskScrollView.contentOffset = CGPointMake(mainWidth, -self.navigationController.navigationBar.frame.size.height - 20);
    }
    else{
        _taskScrollView.contentOffset = CGPointMake(mainWidth*2, -self.navigationController.navigationBar.frame.size.height - 20);
    }
    
    _onTaskTableView.frame = CGRectMake(0, -20-self.navigationController.navigationBar.frame.size.height, _taskScrollView.frame.size.width, _taskScrollView.frame.size.height);
    [_onTaskTableView reloadData];
    
    _saveTaskTableView.frame = CGRectMake(_taskScrollView.frame.size.width, -20-self.navigationController.navigationBar.frame.size.height, _taskScrollView.frame.size.width, _taskScrollView.frame.size.height);
    [_saveTaskTableView reloadData];
    
    _doneTaskTableView.frame = CGRectMake(_taskScrollView.frame.size.width*2, -20-self.navigationController.navigationBar.frame.size.height, _taskScrollView.frame.size.width, _taskScrollView.frame.size.height);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSMutableArray *arrayTaskList = [NSMutableArray arrayWithArray:[defaults objectForKey:@"taskList"]];
    NSString *isNeedFresh = [defaults objectForKey:@"IsNeedFresh"];
    if (arrayTaskList.count != (_onTaskList.count + _saveTaskList.count + _doneTaskList.count) || [isNeedFresh isEqualToString:@"yes"]) {
        [defaults setObject:@"no" forKey:@"IsNeedFresh"];
...
//    }
    ...
    for (int j=0; j<arrayFile.count; j++)
    {
        ...
        if ([[fileName substringWithRange:NSMakeRange(fileName.length - 3, 3)] isEqualToString:@"xml"]) {
    ...
    //xml要最后传
    if (xmlFileName) {
        NSString *fileFullPath = [sourceDir stringByAppendingPathComponent:xmlFileName];
        NSURL *fileUrl = [[NSURL alloc] initFileURLWithPath:fileFullPath isDirectory:NO];
        NSData *data = [NSData dataWithContentsOfURL:fileUrl];
        if ([[xmlFileName substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"a_"]) {//已上传过的物理文件会被改名，加过前缀a_,记录已上传的大小
            self.uploadedFileSize += data.length;
        }
        else
        {
    ...
    for (int k=0; k<onTaskFileFullPathList.count; k++)
    {
        ...
        if ([[fileFullPath substringWithRange:NSMakeRange(fileFullPath.length - 3, 3)] isEqualToString:@"xml"])
            [[FTPHelper sharedInstance] upload:fileUpUrl svrDir:nil];
        else
...
- (void)progressAtPercent:(NSNumber *)uploadedSize
{
    ...
    TaskCustomCell *cell = (TaskCustomCell*)[_onTaskTableView viewWithTag:101];
    if (cell) {
        cell.progressView.progress = prog;
        cell.labelStatus.text = [NSString stringWithFormat:@"%.0f％",prog*100];
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        {
            [[FTPHelper sharedInstance] stopUpload:YES];
            ...
            [_saveTaskTableView reloadData];
            [_doneTaskTableView reloadData];
            [[FTPHelper sharedInstance] stopUpload:NO];
        }
        else if (tableView == _saveTaskTableView)
        {
            _indexSave = indexPath.row;
            UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"发送" otherButtonTitles:@"删除",@"编辑", nil];
            actionSheet.tag = 18;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        cell.labelStatus.text = @"100%";
        cell.progressView.progress = 1.0;
    }
    
    NSString *taskTitle = [dicTemp objectForKey:@"taskTitle"];
    //NSString *taskDate = [dicTemp objectForKey:@"taskDate"];
    ...
    cell.labelTitle.text = taskTitle;
    ...
    //cell.labelStatus.text = taskProgressStatus;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet.tag == 18) {
        ...
        else if (buttonIndex == 2) //编辑
        {
            ...
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:@"edit" forKey:@"isEdit"];
            [defaults setObject:taskDate forKey:@"editTask"];
            [defaults synchronize];
