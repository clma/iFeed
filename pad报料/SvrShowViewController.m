...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *itemAdd = [[UIBarButtonItem alloc] initWithTitle:@"添加" style:UIBarButtonItemStylePlain target:self action:@selector(svrAdd)];
    self.navigationItem.rightBarButtonItem = itemAdd;
    
    //监视设备的方向变化
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(svrShowFrameChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];
    
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;
    
    //服务器列表
    svrTableView = [[UITableView alloc] initWithFrame:CGRectMake(1, 1, mainWidth - 2, mainHeight) style:UITableViewStyleGrouped];
...
- (void)svrShowFrameChanged:(NSNotification*)notification
{
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;

    svrTableView.frame = CGRectMake(1, 1, mainWidth - 2, mainHeight);
}
...
{
    static NSString *cellIdentifier = @"svrShowCell";
