...
- (void)viewDidLoad
{
    ...
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:0.0 green:181.0f/255.0 blue:198.0f/255.0 alpha:1.0];
    
    //建两个按钮
    UIBarButtonItem *btnSave = [[UIBarButtonItem alloc] initWithTitle:@"保存" style:UIBarButtonItemStylePlain target:self action:@selector(saveTaskbtnClicked)];
    btnSave.tag = 11;
    UIBarButtonItem *btnSend = [[UIBarButtonItem alloc] initWithTitle:@"发送" style:UIBarButtonItemStylePlain target:self action:@selector(sendTaskbtnClicked)];
    btnSend.tag = 12;
    self.navigationItem.leftBarButtonItem = btnSave;
    self.navigationItem.rightBarButtonItem = btnSend;
    
    //监视设备的方向变化
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(frameChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];
    ...
    //标题
    titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, self.navigationController.navigationBar.frame.size.height + 40, 80, 40)];
    titleLabel.text = @"标       题";
    [self.view addSubview:titleLabel];
    
    titleTextField = [[UITextField alloc] initWithFrame:CGRectMake(titleLabel.frame.origin.x + titleLabel.frame.size.width, titleLabel.frame.origin.y, MainWidth - titleLabel.frame.origin.x - titleLabel.frame.size.width - 50, 40)];
    titleTextField.layer.cornerRadius = 6.0f;
    titleTextField.layer.borderWidth = 1.0;
    titleTextField.layer.borderColor = [UIColor lightGrayColor].CGColor;
    [self.view addSubview:titleTextField];
    
    contentLabel = [[UILabel alloc] initWithFrame:CGRectMake(titleLabel.frame.origin.x, titleLabel.frame.origin.y + titleLabel.frame.size.height + 30, titleLabel.frame.size.width, 30)];
    contentLabel.text = @"内       容";
    [self.view addSubview:contentLabel];
    
    //先创建描述框
    //if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5.0) {
    //}
    CGRect textDespRect = CGRectMake(titleTextField.frame.origin.x, titleTextField.frame.origin.y + titleTextField.frame.size.height + 20, titleTextField.frame.size.width, (MainHeight/5));
    ...
    textDespView.layer.cornerRadius = 6.0f;
    ...
    textDespView.font = [UIFont fontWithName:@"Arial" size:19.0];
...
//    [textDespView addSubview:_textPlaceholder];
    ...
    imgView.frame = CGRectMake(textDespView.frame.origin.x, textDespView.frame.origin.y + textDespView.frame.size.height + 15, titleTextField.frame.size.width, 1);
    //imgView.backgroundColor = [UIColor lightGrayColor];
    imgView.itemWidth = 89.0;
    imgView.itemHeight = 89.0;
...
//    UIDeviceOrientation curOrientation = [[UIDevice currentDevice] orientation];
    ...
    titleLabel.frame = CGRectMake(40, self.navigationController.navigationBar.frame.size.height + 40, 80, 40);
    titleTextField.frame = CGRectMake(titleLabel.frame.origin.x + titleLabel.frame.size.width, titleLabel.frame.origin.y, MainWidth - titleLabel.frame.origin.x - titleLabel.frame.size.width - 50, 40);
    contentLabel.frame = CGRectMake(titleLabel.frame.origin.x, titleLabel.frame.origin.y + titleLabel.frame.size.height + 30, titleLabel.frame.size.width, 30);
    textDespView.frame = CGRectMake(titleTextField.frame.origin.x, titleTextField.frame.origin.y + titleTextField.frame.size.height + 20, titleTextField.frame.size.width, (MainHeight/5));
    imgView.isNeedLayout = YES;
    imgView.frame = CGRectMake(textDespView.frame.origin.x, textDespView.frame.origin.y + textDespView.frame.size.height + 20, titleTextField.frame.size.width, 1);

//    float angle = 0.0;
//    if (curOrientation == UIDeviceOrientationLandscapeLeft || (curOrientation == UIDeviceOrientationFaceUp && _preOrientation == UIDeviceOrientationLandscapeLeft)) {
//        angle = 90;
//    }
//    else if (curOrientation == UIDeviceOrientationLandscapeRight || (curOrientation == UIDeviceOrientationFaceUp && _preOrientation == UIDeviceOrientationLandscapeRight)){
//        angle = -90;
//    }
//    else if (curOrientation == UIDeviceOrientationPortrait || (curOrientation == UIDeviceOrientationFaceUp && _preOrientation == UIDeviceOrientationPortrait)){
//        angle = 0;
//    }
//    else if (curOrientation == UIDeviceOrientationPortraitUpsideDown || (curOrientation == UIDeviceOrientationFaceUp && _preOrientation == UIDeviceOrientationPortraitUpsideDown)){
//        angle = 180;
//    }
    
    //_preOrientation = curOrientation;
    
    //解决正在播放的视频旋转问题
    UIWindow *window=[UIApplication sharedApplication].keyWindow;
    if (_movPlay) {
        //_movPlay.view.transform = CGAffineTransformMakeRotation(degreesToRadians(angle));
        _movPlay.view.frame = CGRectOffset(window.frame, 0, 0);
    }
    
    //正在查看的图片旋转后
    if (_backgroundView) {
        _backgroundView.frame = CGRectMake(0, 0, MainWidth, MainHeight);
        //_backgroundView.transform = CGAffineTransformMakeRotation(degreesToRadians(angle));
        _backgroundView.frame = CGRectOffset(window.frame, 0, 0);
        
        _myScrollView.frame = CGRectMake(0, 0, _backgroundView.bounds.size.width, _backgroundView.bounds.size.height);
        ...
        _myScrollView.contentOffset = CGPointMake(_pageCtrl.currentPage * _myScrollView.frame.size.width, 0);
        
        //先清楚所有图片，然后只显示当前一张
        for (UIView *imgSubView in _myScrollView.subviews) {
            [imgSubView removeFromSuperview];
        }
        [self loadImgViewToBgView:_pageCtrl.currentPage];
        
        _pageCtrl.frame = CGRectMake(0, _backgroundView.bounds.size.height - 25, _backgroundView.bounds.size.width, 25);
    }
}
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    NSString *isEdit = [defaults objectForKey:@"isEdit"];
    if ([isEdit isEqualToString:@"edit"]) {
        NSString *taskDate = [defaults objectForKey:@"editTask"];
        NSMutableArray *arrayTaskList = [NSMutableArray arrayWithArray:[defaults objectForKey:@"taskList"]];

        if (imgView.imgArray.count > 0) {
            UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"有其他任务在编辑，是否替换?" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"替换" otherButtonTitles:nil];
            actionSheet.tag = 15;
            [actionSheet showInView:self.view];
            return;
        }
        
        [defaults setObject:@"none" forKey:@"isEdit"];
        ...
        for (NSMutableDictionary *dicTemp in arrayTaskList )
        {
            ...
            if ([strDate isEqualToString:taskDate]) {
                //先编辑框和列表中的数据添上
                titleTextField.text = [dicTemp objectForKey:@"taskTitle"];
                textDespView.text = [dicTemp objectForKey:@"taskDesperation"];
                
                NSString *strTarget = [dicTemp objectForKey:@"taskTarget"];
                NSString *strLoc = [dicTemp objectForKey:@"taskLocation"];
                NSString *taskPhone = [dicTemp valueForKey:@"taskPhone"];
                
                [_itemDate setObject:taskDate forKey:@"content"];
                [_itemDicList replaceObjectAtIndex:0 withObject:_itemDate];
                [_itemTarget setObject:strTarget forKey:@"content"];
                [_itemDicList replaceObjectAtIndex:1 withObject:_itemTarget];
                if (strLoc){
                    [_itemPhone setObject:taskPhone forKey:@"content"];
                    [_itemDicList replaceObjectAtIndex:2 withObject:_itemPhone];
                }
                
                if (taskPhone){
                    [_itemLocation setObject:strLoc forKey:@"content"];
                ...
                for (int j=0; j<arrayFile.count; j++)
                {
                    ...
                    NSString *fileType = @"";
                    NSString *strPostFix = [fileName substringWithRange:NSMakeRange(fileName.length - 3, 3)];
                    if ([strPostFix isEqualToString:@"jpg"] || [strPostFix isEqualToString:@"png"]) {
                        fileType = ALAssetTypePhoto;
                    }
                    else if ([strPostFix isEqualToString:@"mov"])
                    {
                        fileType = ALAssetTypeVideo;
                    }
                    else
                        continue;
                    
                    NSMutableDictionary *workingDic = [[NSMutableDictionary alloc] init];
                    [workingDic setObject:fileType forKey:UIImagePickerControllerMediaType];
                    if (fileType == ALAssetTypePhoto) {  //图片
                        UIImage *imgFile = [UIImage imageWithData:data];
                        [workingDic setObject:imgFile forKey:UIImagePickerControllerOriginalImage];
                        [workingDic setObject:[UIImage imageWithCGImage:imgFile.CGImage scale:0.5 orientation:imgFile.imageOrientation] forKey:UIImagePickerControllerEditedImage];
                        
                        [tempArray addObject:[UIImage imageWithCGImage:imgFile.CGImage scale:0.5 orientation:imgFile.imageOrientation]];
                    }
                    else   //视频
                    {
                        ...
                        if (img) {
                            [workingDic setObject:img forKey:UIImagePickerControllerEditedImage];
                            [tempArray addObject:img];
                        }
                        
                        [workingDic setObject:fileUrl forKey:UIImagePickerControllerReferenceURL];
                        [workingDic setObject:data forKey:UIImagePickerControllerMediaMetadata];
                    }
                    
                    [selectImgArray addObject:workingDic];
                }
...
- (void)clearTaskPage
{
    ...
    textDespView.text = @"";
    titleTextField.text = [NSString stringWithFormat:@"默认报料_%@", taskDate];
...
{
    NSString *taskTitle = [dicContent valueForKey:@"taskTitle"];
    ...
    //metaData节点下的标题节点
    GDataXMLElement *attrNode1 = [GDataXMLNode elementWithName:@"sAttribute" stringValue:taskTitle];
    ...
    //metaData节点下的发布时间节点
    NSString *taskCreateDate = taskDate;
    NSArray *arrtime = [taskCreateDate componentsSeparatedByString:@"-"];
    if (arrtime.count >= 5) {
        taskCreateDate = [NSString stringWithFormat:@"%@-%@-%@:%@:%@", [arrtime objectAtIndex:0],[arrtime objectAtIndex:1],[arrtime objectAtIndex:2],[arrtime objectAtIndex:3],[arrtime objectAtIndex:4]];
    }
    else
    {
        NSDateFormatter *formatDate = [[NSDateFormatter alloc] init];
        [formatDate setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        taskCreateDate = [formatDate stringFromDate:[NSDate date]];
    }
    
    GDataXMLElement *attrNode4 = [GDataXMLNode elementWithName:@"sAttribute" stringValue:taskCreateDate];
    ...
    GDataXMLNode *attrName5 = [GDataXMLNode attributeWithName:@"strName" stringValue:@"市"];
    ...
    for (int i=0; i<fileArray.count; i++) {
        ...
        NSInteger Filetype = 100;
        if ([[fileName substringWithRange:NSMakeRange(fileName.length - 3, 3)] isEqualToString:@"mov"]) {
            Filetype = 2;
        }
        else if ([[fileName substringWithRange:NSMakeRange(fileName.length - 3, 3)] isEqualToString:@"amr"])
        {
            Filetype = 6;
        }
        
        GDataXMLNode *contentAttrType = [GDataXMLNode attributeWithName:@"enumType" stringValue:[NSString stringWithFormat:@"%ld",(long)Filetype]];
...
- (void)saveTaskbtnClicked
{
    ...
    NSString *taskTitle = titleTextField.text;
    
    NSString *alertMsg = nil;
    if (taskTitle == nil || [taskTitle isEqualToString:@""]) {
        alertMsg = @"请填写标题！";
    }
    ...
    if (taskTitle)
        [dicTask setObject:taskTitle forKey:@"taskTitle"];
    if (taskDate)
    ...
    [defaults setObject:@"yes" forKey:@"IsNeedFresh"];
    [defaults synchronize];
}

- (void)sendTaskbtnClicked
{
    if (selectImgArray.count <= 0)
        return;
...
- (void)WriteFileToDoc:(NSMutableDictionary*)info
{
    ...
    for (NSMutableDictionary *dict in selectImgArray)
    {
        ...
        NSString *strGuid = [[NSUUID UUID] UUIDString];
        
        //如果是视频
        if ([mediaType isEqualToString:ALAssetTypeVideo])
        {
            NSData *movData = [dict objectForKey:UIImagePickerControllerMediaMetadata];
            NSString *movPath = [sourceDir stringByAppendingPathComponent:[NSString stringWithFormat:@"mov_%@.mov", strGuid]];
            [movData writeToFile:movPath atomically:YES];
            
            [fileArray addObject:[NSString stringWithFormat:@"mov_%@.mov", strGuid]];
        }
        else if ([mediaType isEqualToString:ALAssetTypePhoto] || [mediaType isEqualToString:@"public.image"])
        {
            ...
            if (UIImageJPEGRepresentation(img, 1) != nil)
            {
                ...
                imgPath = [sourceDir stringByAppendingPathComponent:[NSString stringWithFormat:@"img_%@.jpg", strGuid]];
                
                [fileArray addObject:[NSString stringWithFormat:@"img_%@.jpg", strGuid]];
            }
            else
            {
                dataImg = UIImagePNGRepresentation(img);
                imgPath = [sourceDir stringByAppendingPathComponent:[NSString stringWithFormat:@"img_%@.png", strGuid]];
                
                [fileArray addObject:[NSString stringWithFormat:@"img_%@.png", strGuid]];
            }
    ...
    if (xmlDic) {
        ...
        NSString *xmlPath = [sourceDir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.xml",taskDate]];
...
- (void)layoutReset:(CGRect)viewRect
{
    ...
    if ([[UIDevice currentDevice] orientation] == UIDeviceOrientationLandscapeLeft || [[UIDevice currentDevice] orientation] == UIDeviceOrientationLandscapeRight) {
        MainWidth = [[UIScreen mainScreen] bounds].size.height;
        MainHeight = [[UIScreen mainScreen] bounds].size.width;
    }
    
    NSLog(@"layout Reset:%f,%f,%f,%f",viewRect.origin.x, viewRect.origin.y, viewRect.size.width, viewRect.size.height);
    
    CGRect listRect = CGRectMake(titleLabel.frame.origin.x - 14, imgView.frame.origin.y + imgView.frame.size.height + 12, titleLabel.frame.size.width + titleTextField.frame.size.width, 220);
    ...
    if (!tableItemList) {
        ...
        [_itemDate setObject:@"发送时间" forKey:@"keyword"];
        NSDateFormatter *formatDate = [[NSDateFormatter alloc] init];
        [formatDate setDateFormat:@"yyyy-MM-dd HH-mm-ss"];
        NSString *taskDate = [formatDate stringFromDate:[NSDate date]];
        [_itemDate setObject:taskDate forKey:@"content"];
...
//    [actionSheet showInView:self.view];
    ...
    CGRect showRect = CGRectMake(imgView.frame.origin.x + btn.frame.origin.x, imgView.frame.origin.y + btn.frame.origin.y, btn.frame.size.width, btn.frame.size.height);
    if (!popCtrl) {
        popCtrl = [[PopActionSheet alloc] init];
        popCtrl.delegate = self;
    }
    if (!_popoverCtrl) {
        _popoverCtrl = [[UIPopoverController alloc] initWithContentViewController:popCtrl];
        _popoverCtrl.popoverContentSize =CGSizeMake(240,160);
    }
    [_popoverCtrl presentPopoverFromRect:showRect inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (void)selectAlbumClicked
{
    ...
    elcPicker.maximumImagesCount = 17;
...
- (void)imgClicked:(NSInteger)index
{
    ...
    float MainWidth = [[UIScreen mainScreen] bounds].size.width;
    float MainHeight = [[UIScreen mainScreen] bounds].size.height;
    if (curOrientation == UIDeviceOrientationLandscapeLeft || curOrientation == UIDeviceOrientationLandscapeRight || (curOrientation == UIDeviceOrientationFaceUp && (_preOrientation == UIDeviceOrientationLandscapeLeft || _preOrientation == UIDeviceOrientationLandscapeRight))) {
        MainWidth = [[UIScreen mainScreen] bounds].size.height;
        MainHeight = [[UIScreen mainScreen] bounds].size.width;
    }
    ...
    if ([mediaType isEqualToString:ALAssetTypeVideo])
    {
        ...
        [_movPlay.view setFrame:CGRectMake(0, 0, MainWidth, MainHeight)];
    ...
    if (!_backgroundView) {
        ...
        _backgroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MainWidth, MainHeight)];
...
//    }
    ...
    //NSLog(@"edit,init cell %d",indexPath.row);
...
- (void)elcImagePickerController:(ELCImagePickerController *)picker didFinishPickingMediaWithInfo:(NSArray *)info
{
    ...
    //转换成image的array,只支持显示17个资源
    NSMutableArray *tempArray = [[NSMutableArray alloc] initWithCapacity:0];
    for (int k=0; k<info.count; k++) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[info objectAtIndex:k]];
        UIImage *image = [dict objectForKey:UIImagePickerControllerEditedImage];  //-amen
        [tempArray addObject:image];
