...
- (void)viewDidLoad
{
    ...
    //监视设备的方向变化
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setFrameChanged:) name:UIDeviceOrientationDidChangeNotification object:nil];
    
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;
    
    taskTableView = [[UITableView alloc] initWithFrame:CGRectMake(1, 1, mainWidth - 2, mainHeight - 1) style:UITableViewStyleGrouped];
...
- (void)setFrameChanged:(NSNotification*)notification
{
    float mainWidth = [[UIScreen mainScreen] bounds].size.width;
    float mainHeight = [[UIScreen mainScreen] bounds].size.height;
    
    taskTableView.frame = CGRectMake(1, 1, mainWidth - 2, mainHeight - 1);
}
